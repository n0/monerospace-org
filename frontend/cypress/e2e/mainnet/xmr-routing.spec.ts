...
describe('XMR routing contract', () => {
  if (baseModule === 'mempool') {
    ...
    it('keeps active legal and footer surfaces retargeted to monerospace.org', () => {
      ...
      cy.visit('/uswap');
      cy.contains('h1', 'Buy, sell, and swap XMR').should('be.visible');
      cy.contains('onramp into Monero').should('be.visible');
      cy.contains('Off-ramp from XMR').should('be.visible');
      cy.contains('Convert XMR anywhere').should('be.visible');
      cy.get('a[href="https://app.uswap.net/"]').should('contain', 'Open uSwap');
      cy.get('a[href="https://t.me/hiss"]').should('contain', 'Contact Us');
